...
class Sphere {
public:
	static IndexedTriangleList MakeTesselated(custom::VertexLayout layout, int latDiv, int longDiv) {
		...
		custom::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<custom::VertexLayout> layout = std::nullopt) {
		using Element = custom::VertexLayout::ElementType;
		if (!layout) {
			layout = custom::VertexLayout{}.Append(Element::Position3D);
		}
