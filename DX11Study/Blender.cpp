...
namespace Bind {
	...
	std::shared_ptr<Blender> Blender::Resolve(Graphics& gfx, bool blending, std::optional<float> factors) {
		return Codex::Resolve<Blender>(gfx, blending, factors);
	}

	std::string Blender::GenerateUID(bool blending, std::optional<float> factors) {
		using namespace std::string_literals;
		return typeid(Blender).name() + "#"s + (blending ? "b"s : "n"s) + (factors ? "#f"s + std::to_string(*factors) : "");
	}
