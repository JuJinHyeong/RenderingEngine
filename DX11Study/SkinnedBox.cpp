	...
	TestObject(gfx, rng, adist, ddist, odist, rdist)
{
	...
	if (!IsStaticInitialized()) {
		...
		AddStaticBind(std::make_unique<Texture>(gfx, Surface::FromFile("Images\\younha.jpg")));
		...
		AddStaticBind(std::make_unique<PixelConstantBuffer<PSMaterialConstant>>(gfx, pixelConstant, 1u));
	}
	else {
		SetIndexFromStatic();
	}
