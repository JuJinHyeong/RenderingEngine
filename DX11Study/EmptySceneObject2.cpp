...
void EmptySceneObject2::Submit(size_t channel, const DirectX::FXMMATRIX& accumulatedTransform) const noexcept(!IS_DEBUG) {
	for (const auto& child : childPtrs) {
		child->Submit(channel, localTransform * accumulatedTransform);
	}
}
