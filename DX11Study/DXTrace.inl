...
if (strFile) {
    DX_SPRINTF_S(strBuffer, BUFFER_SIZE, DX_STR_WRAP("%ls( %ls ): "), strFile, strBufferLine);
...
DX_SPRINTF_S(strBufferError, 256, DX_STR_WRAP("%ls (0x%0.8x)"), DX_GETERRORSTRING(hr), hr);
DX_SPRINTF_S(strBuffer, BUFFER_SIZE, DX_STR_WRAP("hr=%ls"), strBufferError);
DX_OUTPUTDEBUGSTRING(strBuffer);
...
if (bPopMsgBox) {
    ...
    if (nMsgLen > 0)
        DX_SPRINTF_S(strBufferMsg, 1024, DX_STR_WRAP("Calling: %ls\n"), strMsg);

    DX_SPRINTF_S(strBuffer, BUFFER_SIZE, DX_STR_WRAP("File: %ls\nLine: %ls\nError Code: %ls\n%lsDo you want to debug the application?"),
